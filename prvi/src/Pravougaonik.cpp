...
{
    Tacka offset = diff(t, DajTjeme(p));
    Postavi( { points [(posToInt(p) + 2) % 4].x, points [(posToInt(p) + 2) % 4].y }, t);
}
...
{
    std::cout << "{{" << DajTjeme(Pozicija::GoreLijevo).x << "," << DajTjeme(Pozicija::GoreLijevo).y << "},{" <<
                        DajTjeme(Pozicija::DoljeDesno).x << "," << DajTjeme(Pozicija::DoljeDesno).y << "}}";
}
